...
/* ══════════════════════════════════════════════
   CONFIGURATION JSONBIN (optionnel)
   Créez un bin sur https://jsonbin.io,
   puis renseignez l'ID et la clé X-Master-Key.
   Laissez vide pour utiliser localStorage.
══════════════════════════════════════════════ */
const JSONBIN_ID  = '69f0a331856a68218980f9f5';   // ex. '6637a8f2acd3cb34a8438f82'
const JSONBIN_KEY = '$2a$10$xd5xi2EL54RAm0FdsrUR5egs5QODha9H6ZsVfWUDt1aT.5nXoZKCO';   // ex. '$2a$10$...'
...
async function loadVotes(year, month) {
  if (JSONBIN_ID && JSONBIN_KEY) {
    return await loadVotesRemote(year, month);
  }
...
async function saveVotes(year, month, votes) {
  if (JSONBIN_ID && JSONBIN_KEY) {
    return await saveVotesRemote(year, month, votes);
  }
...
async function saveAllRemote(data) {
  try {
...
async function loadVotesRemote(year, month) {
  const all = await loadAllRemote();
  return all[voteKey(year, month)] || {};
}

async function saveVotesRemote(year, month, votes) {
  const all = await loadAllRemote();
  all[voteKey(year, month)] = votes;
  await saveAllRemote(all);
}
